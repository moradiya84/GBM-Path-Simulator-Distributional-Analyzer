...
DeltaHedger::backtest_path_attributed(const TimeGrid &grid,
  ...
  for (size_t i = 1; i <= num_steps; ++i) {
    ...
    // financing pnl is how much your hedge earns (or pays) just from funding
    ...
    // Theta is the option's time-decay rate: dV ≈ theta * dt.
    //
    /* We are short the option (we sold it earlier and received premium),
    but the position is still "alive": we could buy it back at the current
    market price to close the position. Therefore, the option's price change
    directly affects our PnL.
    Short position PnL = -(change in option value).
    So if the option price decreases (time decay), we profit;
    if it increases, we lose.

    Hence, theta PnL contribution = -theta * dt.*/
